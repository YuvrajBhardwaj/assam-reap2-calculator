...
import { fetchAreaTypes, requestChange, getHistory } from '@/services/masterDataService';
import { AuditLog, BaseEntity } from '@/types/masterData';
import { useToast } from '@/hooks/use-toast';

type AreaTypeEntity = BaseEntity & { areaTypesGenId?: string };

const AreaTypesDeptTable: React.FC = () => {
  const { toast } = useToast();
  const [rows, setRows] = useState<AreaTypeEntity[]>([]);
  ...
  const [updateRow, setUpdateRow] = useState<AreaTypeEntity | null>(null);
  const [updateName, setUpdateName] = useState('');
  const [updateReason, setUpdateReason] = useState('');

  const [deactivateOpen, setDeactivateOpen] = useState(false);
  const [deactivateRow, setDeactivateRow] = useState<AreaTypeEntity | null>(null);
  ...
  const refresh = async () => {
    try {
      setLoading(true);
      const res = await fetchAreaTypes();
      const items = Array.isArray(res?.data) ? res.data : [];
      const mapped: AreaTypeEntity[] = items.map((it: any) => ({
        id: (it.areaTypesGenId ?? it.id ?? '').toString(),
        code: (it.areaTypesGenId ?? it.code ?? '').toString(),
        name: it.areaType ?? it.name ?? '',
        isActive: it.active ?? it.isActive ?? true,
      }));
      setRows(mapped);
    } catch {
      toast({ title: 'Failed to fetch area types', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const submitCreate = async () => {
    try {
      await requestChange('Area Types', 'CREATE', { areaType: createName }, createReason);
  ...
  const submitUpdate = async () => {
    if (!updateRow) return;
    try {
      await requestChange('Area Types', 'UPDATE', { id: updateRow.id, areaType: updateName }, updateReason);
  ...
  const submitDeactivate = async () => {
    ...
    try {
      await requestChange('Area Types', 'DEACTIVATE', { id: deactivateRow.id }, deactivateReason);
  ...
  const openHistory = async (row: AreaTypeEntity) => {
    try {
      const logs = await getHistory('Area Types', String(row.id));
      setHistoryLogs(logs || []);
      setHistoryOpen(true);
    } catch {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Table>
          ...
          <TableBody>
            ...
            {!rows.length && (
              <TableRow>
                <TableCell colSpan={4} className="text-center">No area types found.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Request Create Area Type</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <Input value={createName} onChange={(e) => setCreateName(e.target.value)} placeholder="Area Type Name" />
      ...
      <Dialog open={updateOpen} onOpenChange={setUpdateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Request Update Area Type</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <Input value={updateName} onChange={(e) => setUpdateName(e.target.value)} placeholder="Area Type Name" />
      ...
      <Dialog open={deactivateOpen} onOpenChange={setDeactivateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Request Deactivate Area Type</DialogTitle></DialogHeader>
          <div className="space-y-3">
      ...
      <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Area Type Change History</DialogTitle></DialogHeader>
...
export default AreaTypesDeptTable;