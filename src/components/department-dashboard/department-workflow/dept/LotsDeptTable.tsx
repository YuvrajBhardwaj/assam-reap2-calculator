...
import { fetchLotsByDistrictAndCircle, requestChange, getHistory } from '@/services/masterDataService';
import { Circle, District, Lot, AuditLog } from '@/types/masterData';
import { useToast } from '@/hooks/use-toast';

const LotsDeptTable: React.FC = () => {
  ...
  const [updateRow, setUpdateRow] = useState<Lot | null>(null);
  const [updateName, setUpdateName] = useState('');
  const [updateReason, setUpdateReason] = useState('');

  const [deactivateOpen, setDeactivateOpen] = useState(false);
  const [deactivateRow, setDeactivateRow] = useState<Lot | null>(null);
  ...
  useEffect(() => {
    (async () => {
      if (selectedDistrict && selectedCircle) {
        setLoading(true);
        setRows(await fetchLotsByDistrictAndCircle(selectedDistrict, selectedCircle));
        setLoading(false);
      } else {
        setRows([]);
      }
  ...
  const submitCreate = async () => {
    ...
    try {
      await requestChange('Lots', 'CREATE', { districtCode: selectedDistrict, circleCode: selectedCircle, lotName: createName }, createReason);
  ...
  const submitUpdate = async () => {
    if (!updateRow) return;
    try {
      await requestChange('Lots', 'UPDATE', { id: updateRow.id, districtCode: updateRow.districtCode, circleCode: updateRow.circleCode, lotName: updateName }, updateReason);
  ...
  const submitDeactivate = async () => {
    ...
    try {
      await requestChange('Lots', 'DEACTIVATE', { id: deactivateRow.id }, deactivateReason);
  ...
  const openHistory = async (row: Lot) => {
    try {
      const logs = await getHistory('Lots', String(row.id));
      setHistoryLogs(logs || []);
      setHistoryOpen(true);
    } catch {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Table>
          ...
          <TableBody>
            ...
            {!rows.length && (
              <TableRow>
                <TableCell colSpan={4} className="text-center">No lots found.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Request Create Lot</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <Input value={createName} onChange={(e) => setCreateName(e.target.value)} placeholder="Lot Name" />
      ...
      <Dialog open={updateOpen} onOpenChange={setUpdateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Request Update Lot</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <Input value={updateName} onChange={(e) => setUpdateName(e.target.value)} placeholder="Lot Name" />
      ...
      <Dialog open={deactivateOpen} onOpenChange={setDeactivateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Request Deactivate Lot</DialogTitle></DialogHeader>
          <div className="space-y-3">
      ...
      <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Lot Change History</DialogTitle></DialogHeader>
