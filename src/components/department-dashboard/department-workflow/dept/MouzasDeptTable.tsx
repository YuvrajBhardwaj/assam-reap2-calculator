...
import { Circle, District, Mouza, AuditLog } from '@/types/masterData';
import { useToast } from '@/hooks/use-toast';

const MouzasDeptTable: React.FC = () => {
  const { toast } = useToast();
  const [districts, setDistricts] = useState<District[]>([]);
  const [selectedDistrict, setSelectedDistrict] = useState<string>('');
  const [circles, setCircles] = useState<Circle[]>([]);
  const [selectedCircle, setSelectedCircle] = useState<string>('');
  const [rows, setRows] = useState<Mouza[]>([]);
  ...
  const [updateRow, setUpdateRow] = useState<Mouza | null>(null);
  const [updateName, setUpdateName] = useState('');
  const [updateReason, setUpdateReason] = useState('');

  const [deactivateOpen, setDeactivateOpen] = useState(false);
  const [deactivateRow, setDeactivateRow] = useState<Mouza | null>(null);
  ...
  const loadCircles = async (districtCode: string) => {
    try {
      const list = await getCirclesByDistrict(districtCode);
      setCircles(list);
    } catch {}
  };
  ...
  const refresh = async () => {
    ...
    try {
      setLoading(true);
      const list = await getMouzasByDistrictAndCircle(selectedDistrict, selectedCircle);
      setRows(list);
    } catch {
      toast({ title: 'Failed to fetch mouzas', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => { if (selectedDistrict) loadCircles(selectedDistrict); setSelectedCircle(''); }, [selectedDistrict]);
  useEffect(() => { refresh(); }, [selectedCircle]);
  ...
  const submitCreate = async () => {
    ...
    try {
      await requestChange('Mouzas', 'CREATE', { districtCode: selectedDistrict, circleCode: selectedCircle, mouzaName: createName }, createReason);
  ...
  const submitUpdate = async () => {
    if (!updateRow) return;
    try {
      await requestChange('Mouzas', 'UPDATE', { id: updateRow.id, districtCode: updateRow.districtCode, circleCode: updateRow.circleCode, mouzaName: updateName }, updateReason);
  ...
  const submitDeactivate = async () => {
    ...
    try {
      await requestChange('Mouzas', 'DEACTIVATE', { id: deactivateRow.id }, deactivateReason);
  ...
  const openHistory = async (row: Mouza) => {
    try {
      const logs = await getHistory('Mouzas', String(row.id));
      setHistoryLogs(logs || []);
      setHistoryOpen(true);
    } catch {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <Table>
          ...
          <TableBody>
            ...
            {!rows.length && (
              <TableRow>
                <TableCell colSpan={4} className="text-center">No mouzas found.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Request Create Mouza</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <Input value={createName} onChange={(e) => setCreateName(e.target.value)} placeholder="Mouza Name" />
      ...
      <Dialog open={updateOpen} onOpenChange={setUpdateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Request Update Mouza</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <Input value={updateName} onChange={(e) => setUpdateName(e.target.value)} placeholder="Mouza Name" />
      ...
      <Dialog open={deactivateOpen} onOpenChange={setDeactivateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Request Deactivate Mouza</DialogTitle></DialogHeader>
          <div className="space-y-3">
      ...
      <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Mouza Change History</DialogTitle></DialogHeader>
...
export default MouzasDeptTable;